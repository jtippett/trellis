require "../../src/trellis"
require "../../src/trellis/http/adapter"
...
class FakeAdapter
  ...
  def call(request : Trellis::HTTP::Request) : Trellis::HTTP::Response
    @called = true
    body = @body
    raise "FakeAdapter called without a configured response" if body.nil?
    Trellis::HTTP::Response.new(@status || 200, HTTP::Headers.new, body)
  end
end
...
class CountingAdapter
  ...
  def call(request : Trellis::HTTP::Request) : Trellis::HTTP::Response
    ...
    @calls += 1
    Trellis::HTTP::Response.new(status, @headers, %({"ok":true}))
  end
end
