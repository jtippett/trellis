...
describe ReqLLM::HTTP::Pipeline do
  describe "retry-aware perform" do
    it "retries on 5xx then returns the first non-retryable response" do
      ...
      req.retry = ReqLLM::RetryPolicy.new(max_retries: 3, base_delay: Time::Span.zero)
      adapter = CountingAdapter.new([503, 503, 200])

      resp = ReqLLM::HTTP::Pipeline.perform(req, adapter)
    ...
    it "stops retrying once max_retries is exhausted" do
      req = ReqLLM::HTTP::Request.new("POST", URI.parse("https://x/y"))
      req.retry = ReqLLM::RetryPolicy.new(max_retries: 2, base_delay: Time::Span.zero)
    ...
    it "does not retry a 200 response" do
      ...
      ReqLLM::HTTP::Pipeline.perform(req, adapter)
    ...
    it "honors a numeric Retry-After header over the backoff policy" do
      ...
      resp = ReqLLM::HTTP::Response.new(429, headers, "")

      # Asserts the computed wait, not an actual sleep: 5 seconds from the header.
      ReqLLM::HTTP::Pipeline.retry_delay(resp, policy, 0).should eq(5.seconds)
    end
    ...
    it "falls back to policy backoff when no Retry-After header is present" do
      ...
      resp = ReqLLM::HTTP::Response.new(503, HTTP::Headers.new, "")

      ReqLLM::HTTP::Pipeline.retry_delay(resp, policy, 1).should eq(20.milliseconds)
    end
  end
end
