...
private class BaseProviderStub < ReqLLM::BaseProvider
  def id : String
    "stub"
  end
...
private def response_step_names(req : ReqLLM::HTTP::Request) : Array(Symbol)
  req.response_steps.map { |(name, _)| name }
end

private def build_request : ReqLLM::HTTP::Request
  req = ReqLLM::HTTP::Request.new("POST", URI.parse("https://stub.test/v1"))
  req.model = LLMDB::Model.new("stub", "stub-model")
  req
end

describe ReqLLM::BaseProvider do
  describe "#attach" do
