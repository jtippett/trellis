...
describe ReqLLM::Providers::Google do
  describe "#decode_response" do
    it "decodes a basic generateContent response into text, finish_reason, and usage" do
      ...
      resp = ReqLLM::Fixture.load_response("spec/fixtures/google/chat_basic.json")
    ...
    it "decodes functionCall parts into tool_calls and overrides STOP to ToolCalls" do
      provider = ReqLLM::Providers::Google.new
      resp = ReqLLM::Fixture.load_response("spec/fixtures/google/chat_tools.json")
      _, resp = provider.decode_response(google_req, resp)
...
# replay).
describe "ReqLLM.generate_text (google, offline fixture)" do
  it "returns a costed Response from a recorded fixture with no API key set" do
    ...
    resp = ReqLLM.generate_text("google:gemini-2.0-flash", "Hi", fixture: "chat_basic")
