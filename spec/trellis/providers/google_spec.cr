...
describe Trellis::Providers::Google do
  describe "#encode_chat_body" do
    it "encodes a basic chat body matching the canonical golden" do
      ...
      body = Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts)
    ...
    it "encodes sampling params matching the canonical golden" do
      ...
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate({temperature: 0.7, top_p: 0.9, max_tokens: 256, stop: ["END"]})
      body = Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts)
    ...
    it "deep-strips $schema and additionalProperties from tool parameters at every level" do
      ...
      tool = Trellis::Tool.new("search", "Search", schema)

      opts = Trellis::Options.validate({tools: [tool]})
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "encodes an assistant message with role \"model\"" do
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::User, "Hi"),
        Trellis::Message.new(Trellis::Role::Assistant, "Hello!"),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "encodes an assistant message with tool_calls as a functionCall part with DECODED args" do
      ...
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::User, "Weather in Paris?"),
        Trellis::Message.new(Trellis::Role::Assistant, "Let me check.", tool_calls: [tc]),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "folds two consecutive tool messages into ONE user entry with two functionResponse parts" do
      tc1 = Trellis::ToolCall.new("call_1", "get_weather", %({"city":"Paris"}))
      tc2 = Trellis::ToolCall.new("call_2", "get_conditions", %({"city":"Paris"}))
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::User, "Weather?"),
        Trellis::Message.new(Trellis::Role::Assistant, "", tool_calls: [tc1, tc2]),
        Trellis::Message.new(Trellis::Role::Tool, "72F", name: "get_weather", tool_call_id: "call_1"),
        Trellis::Message.new(Trellis::Role::Tool, "Sunny", name: "get_conditions", tool_call_id: "call_2"),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "emits ONLY functionCall parts for an assistant with empty text + tool_calls (no empty text part)" do
      tc = Trellis::ToolCall.new("call_1", "get_weather", %({"city":"Paris"}))
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::User, "Weather?"),
        Trellis::Message.new(Trellis::Role::Assistant, "", tool_calls: [tc]),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "encodes a bare empty user message as a single empty-text part" do
      ctx = Trellis::Context.new([Trellis::Message.new(Trellis::Role::User, "")])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "uses \"unknown\" as the functionResponse name when the message has none" do
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::Tool, "result", tool_call_id: "call_1"),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)
      parsed = JSON.parse(Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts))
    ...
    it "raises when a tool message has no tool_call_id" do
      ctx = Trellis::Context.new([
        Trellis::Message.new(Trellis::Role::Tool, "result"),
      ])
      model = LLMDB.model("google:gemini-2.0-flash")
      opts = Trellis::Options.validate(NamedTuple.new)

      expect_raises(Trellis::Error::Invalid::Parameter, /tool_call_id/) do
        Trellis::Providers::Google.new.encode_chat_body(model, ctx, opts)
      end
    end
  end
  ...
  describe "#encode_object_body" do
    ...
    it "uses responseSchema with UPPERCASE types for a pre-2.5 model (gemini-2.0-flash)" do
      ...
      body = Trellis::Providers::Google.new.encode_object_body(
    ...
    it "raises on a tuple-array (list-valued items) schema on the responseSchema path" do
      ctx = Trellis::Context.new([Trellis::Message.new(Trellis::Role::User, "x")])
      ...
      expect_raises(Trellis::Error::Invalid::Schema, /tuple/) do
        Trellis::Providers::Google.new.encode_object_body(model, ctx, opts, tuple_schema, "output_schema")
      end
    end
  ...
  describe "registration" do
    it "registers itself under the \"google\" id" do
      Trellis::Registry.fetch("google").should be_a(Trellis::Providers::Google)
    end
  end
  ...
  describe "#prepare_request" do
    it "builds a POST to /models/<id>:generateContent carrying typed state" do
      ...
      opts = Trellis::Options.validate({temperature: 0.7})
      req = Trellis::Providers::Google.new.prepare_request(:chat, model, ctx, opts)
    ...
    it "raises when model.provider does not match the provider id" do
      ...
      expect_raises(Trellis::Error::Invalid::Parameter, /provider/) do
        Trellis::Providers::Google.new.prepare_request(:chat, model, ctx, opts)
      end
    end
  end
  ...
  describe "auth headers" do
    ...
    it "AUTH-SKIP-ON-REPLAY: does not resolve a key when replaying an existing fixture" do
      ...
      begin
        file = Trellis::Fixture.path(:google, "chat_basic")
