...
private def anthropic_response(body : String) : Trellis::HTTP::Response
...
private def anthropic_req
  ...
  req = Trellis::HTTP::Request.new("POST", URI.parse("https://api.anthropic.com/v1/messages"))
  req.model = model
...
describe Trellis::Providers::Anthropic do
  describe "#decode_response" do
    it "decodes a basic Messages response into text, finish_reason, and usage" do
      ...
      resp = Trellis::Fixture.load_response("spec/fixtures/anthropic/chat_basic.json")
    ...
    it "decodes tool_use blocks into Response#tool_calls with ToolCalls finish" do
      provider = Trellis::Providers::Anthropic.new
      resp = Trellis::Fixture.load_response("spec/fixtures/anthropic/chat_tools.json")
      _, resp = provider.decode_response(anthropic_req, resp)
  ...
  describe "#decode_response stream parity" do
    it "produces exactly one text part for a text-only response (matching accumulator finish)" do
      ...
      acc << Trellis::StreamChunk.text("Hello world")
...
# replay).
describe "Trellis.generate_text (anthropic, offline fixture)" do
  it "returns a costed Response from a recorded fixture with no API key set" do
    saved = ENV["ANTHROPIC_API_KEY"]?
    ENV.delete("ANTHROPIC_API_KEY")

    resp = Trellis.generate_text("anthropic:claude-3-5-sonnet-20241022", "Hi", fixture: "chat_basic")
