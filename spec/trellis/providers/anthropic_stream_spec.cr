...
private def event(data : String) : Trellis::SSE::Event
  Trellis::SSE::Event.new(data: data)
end
...
describe Trellis::Providers::Anthropic do
  describe "#decode_stream_event" do
    ...
    it "decodes a content_block_delta text_delta into one Content chunk" do
      ...
      chunks[0].type.should eq(Trellis::ChunkType::Content)
    ...
    it "decodes a message_delta stop_reason into a Meta chunk" do
      ...
      chunks.size.should eq(1)
      chunks[0].type.should eq(Trellis::ChunkType::Meta)
    ...
    it "decodes a message_start usage into a normalized Meta usage chunk" do
      ...
      chunks.size.should eq(1)
      c = chunks[0]
      c.type.should eq(Trellis::ChunkType::Meta)
    ...
    it "decodes a content_block_start tool_use into a ToolCall delta chunk" do
      ...
      chunks.size.should eq(1)
      c = chunks[0]
      c.type.should eq(Trellis::ChunkType::ToolCall)
    ...
    it "decodes a content_block_delta thinking_delta into a Thinking chunk" do
      ...
      chunks.size.should eq(1)
      chunks[0].type.should eq(Trellis::ChunkType::Thinking)
    ...
    it "raises on an in-stream error frame (surfaced to the consumer)" do
      ...
      ex = expect_raises(Trellis::Error::API::Response, /Anthropic stream error/) do
  ...
  # End-to-end: Trellis.stream_text streams tokens offline from a recorded SSE
  ...
  describe "Trellis.stream_text (offline fixture replay)" do
    it "streams ordered content chunks from the recorded SSE fixture with NO key" do
      without_anthropic_key do
        stream = Trellis.stream_text(
