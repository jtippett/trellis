...
describe Trellis::Options do
  describe ".validate (base schema)" do
    ...
    it "raises Invalid::Parameter on a range violation" do
      expect_raises(Trellis::Error::Invalid::Parameter, /temperature/) do
        Trellis::Options.validate({temperature: 3.0})
      end
    end

    it "raises Invalid::Parameter on a type mismatch" do
      expect_raises(Trellis::Error::Invalid::Parameter, /max_tokens/) do
        Trellis::Options.validate({max_tokens: "lots"})
      end
    end

    it "raises Invalid::Parameter on an unknown key" do
      expect_raises(Trellis::Error::Invalid::Parameter, /unknown/) do
        Trellis::Options.validate({not_a_real_option: 1})
      end
    end

    it "coerces an integer where a float is expected" do
      v = Trellis::Options.validate({temperature: 1})
      v.fetch_float?(:temperature).should eq(1.0)
    end

    it "exposes validated values via [] and get" do
      v = Trellis::Options.validate({temperature: 0.5})
  ...
  describe "sampling params" do
    ...
    it "raises on out-of-range top_p" do
      expect_raises(Trellis::Error::Invalid::Parameter, /top_p/) do
        Trellis::Options.validate({top_p: 1.5})
      end
    end

    it "raises on out-of-range frequency_penalty" do
      expect_raises(Trellis::Error::Invalid::Parameter, /frequency_penalty/) do
        Trellis::Options.validate({frequency_penalty: 3.0})
      end
    end

    it "raises on out-of-range presence_penalty" do
      expect_raises(Trellis::Error::Invalid::Parameter, /presence_penalty/) do
        Trellis::Options.validate({presence_penalty: -3.0})
      end
    end

    it "accepts stop as a String" do
      v = Trellis::Options.validate({stop: "END"})
      v.fetch_stop(:stop).should eq("END")
    end

    it "accepts stop as an Array(String)" do
      v = Trellis::Options.validate({stop: ["END", "STOP"]})
      v.fetch_stop(:stop).should eq(["END", "STOP"])
    end

    it "rejects stop of the wrong type" do
      expect_raises(Trellis::Error::Invalid::Parameter, /stop/) do
        Trellis::Options.validate({stop: 42})
      end
    end
  end
  ...
  describe "schema extension" do
    it "merges provider-specific keys onto the base schema" do
      schema = Trellis::Options.base_schema.merge(
        Trellis::Options.schema({
    ...
    it "applies a provider default when the key is omitted" do
      schema = Trellis::Options.base_schema.merge(
        Trellis::Options.schema({reasoning_effort: {type: :symbol, default: :medium}})
      )
    ...
    it "still rejects unknown keys after extension" do
      schema = Trellis::Options.base_schema.merge(
        Trellis::Options.schema({reasoning_effort: {type: :symbol}})
      )
      expect_raises(Trellis::Error::Invalid::Parameter) do
