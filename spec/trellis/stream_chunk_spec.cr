...
describe Trellis::StreamChunk do
  it ".text builds a content chunk carrying text" do
    chunk = Trellis::StreamChunk.text("Hello world")
    chunk.type.should eq(Trellis::ChunkType::Content)
    chunk.text.should eq("Hello world")
  end

  it ".thinking builds a thinking chunk carrying text" do
    chunk = Trellis::StreamChunk.thinking("Let me think...")
    chunk.type.should eq(Trellis::ChunkType::Thinking)
    chunk.text.should eq("Let me think...")
  end

  it ".tool_call builds a tool_call chunk with name and arguments" do
    chunk = Trellis::StreamChunk.tool_call("get_weather", {"city" => JSON::Any.new("NYC")})
    chunk.type.should eq(Trellis::ChunkType::ToolCall)
  ...
  it ".tool_call_delta builds a tool_call chunk with the accumulator's delta metadata" do
    chunk = Trellis::StreamChunk.tool_call_delta(
  ...
  it ".tool_call_delta omits absent optional keys (index always present)" do
    chunk = Trellis::StreamChunk.tool_call_delta(1, arguments_fragment: %(ation"}))
  ...
  it ".meta builds a meta chunk and merges extra metadata" do
    chunk = Trellis::StreamChunk.meta(
      {"finish_reason" => JSON::Any.new("stop")},
      {"model" => JSON::Any.new("gpt-4o-mini")}
    )
    chunk.type.should eq(Trellis::ChunkType::Meta)
