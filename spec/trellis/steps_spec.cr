...
private def build_request
  Trellis::HTTP::Request.new("POST", URI.parse("https://x/y"))
end
...
private class UsageStubProvider < Trellis::BaseProvider
  def id : String
    "openai"
  end
...
describe Trellis::Steps do
  describe ".error" do
    it "is a named response step keyed :error" do
      name, _step = Trellis::Steps.error
      name.should eq(:error)
    end

    it "raises Error::API::Request carrying status and body for 4xx" do
      req = build_request
      resp = Trellis::HTTP::Response.new(401, HTTP::Headers.new, %({"error":"nope"}))
      _name, step = Trellis::Steps.error

      ex = expect_raises(Trellis::Error::API::Request) do
        step.call(req, resp)
      end
    ...
    it "raises for 5xx as well" do
      req = build_request
      resp = Trellis::HTTP::Response.new(503, HTTP::Headers.new, "down")
      _name, step = Trellis::Steps.error
      expect_raises(Trellis::Error::API::Request) { step.call(req, resp) }
    end

    it "passes through a 2xx response unchanged" do
      req = build_request
      resp = Trellis::HTTP::Response.new(200, HTTP::Headers.new, %({"ok":true}))
      _name, step = Trellis::Steps.error
    ...
    it "attaches to a request's response steps under :error" do
      req = build_request
      Trellis::Steps.attach_error(req)
  ...
  describe ".usage" do
    it "is a named response step keyed :usage" do
      name, _step = Trellis::Steps.usage
    ...
    it "preserves the token usage that decode attached to the decoded response" do
      ...
      resp.decoded = Trellis::Response.new(
        ...
        usage: Trellis::Usage.new(input_tokens: 10, output_tokens: 5),
      )
      _name, step = Trellis::Steps.usage
    ...
    it "passes through when there is no decoded response" do
      req = build_request
      resp = Trellis::HTTP::Response.new(200, HTTP::Headers.new, "")
      _name, step = Trellis::Steps.usage
    ...
    it "attaches to a request's response steps under :usage" do
      req = build_request
      Trellis::Steps.attach_usage(req)
    ...
    it "computes and stores per-token cost from req.model pricing via provider.extract_usage" do
      ...
      resp.decoded = Trellis::Response.new(
        model: "openai:gpt-4o-mini",
        usage: Trellis::Usage.new(input_tokens: 11, output_tokens: 7),
      )

      _name, step = Trellis::Steps.usage(provider)
      _out_req, out_resp = step.call(req, resp)
