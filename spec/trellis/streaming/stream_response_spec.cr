...
private def content(text : String) : Trellis::StreamChunk
  Trellis::StreamChunk.text(text)
end

# Builds a terminal meta chunk with finish_reason + usage.
private def meta(finish_reason : String, input : Int32, output : Int32) : Trellis::StreamChunk
  usage = {
  ...
  Trellis::StreamChunk.meta(data)
end

# A StreamResponse whose producer emits a fixed list of chunks, in order.
private def fixed_stream(chunks : Array(Trellis::StreamChunk),
                         model : String = "test:model",
                         context : Trellis::Context? = nil) : Trellis::StreamResponse
  Trellis::StreamResponse.new(model, context) do |emit|
    chunks.each { |c| emit.call(c) }
  end
end

describe Trellis::StreamResponse do
  describe "#each" do
    it "yields all chunks in order and to_a returns them" do
      ...
      within do
        ...
        received[3].type.should eq(Trellis::ChunkType::Meta)
      end
    end
    ...
    it "raises on a second consumption (single-consume contract)" do
      ...
      expect_raises(Trellis::StreamResponse::AlreadyConsumed) do
        stream.each { |_| }
      end
    end
  end
  ...
  describe "#join" do
    it "folds chunks into a Response with concatenated text + meta" do
      ...
      within do
        ...
        response.finish_reason.should eq(Trellis::FinishReason::Stop)
    ...
    it "threads the input context into the merged Response context" do
      ...
      within do
        ...
        msgs.last.role.should eq(Trellis::Role::Assistant)
      end
    end
  end
  ...
  describe "error propagation" do
    it "re-raises a producer error to an #each consumer (no hang)" do
      stream = Trellis::StreamResponse.new("test:model", nil) do |emit|
  ...
  describe "#cancel" do
    it "lets a consumer abandon the stream without hanging the producer" do
      ...
      stream = Trellis::StreamResponse.new("test:model", nil, capacity: 2) do |emit|
