...
module Trellis
  ...
  module StreamAdapter
    ...
    def drive(req : HTTP::Request, provider : Trellis::Provider,
              emit : Trellis::StreamChunk ->) : Nil
    ...
    # live stream would.
    private def replay(req : HTTP::Request, provider : Trellis::Provider,
                       emit : Trellis::StreamChunk ->) : Nil
    ...
    private def live(req : HTTP::Request, provider : Trellis::Provider,
                     emit : Trellis::StreamChunk ->) : Nil
      uri = req.url
    ...
    private def pump(io : IO, provider : Trellis::Provider,
                     emit : Trellis::StreamChunk ->) : Nil
      Trellis::SSE.each_event(io) do |event|
