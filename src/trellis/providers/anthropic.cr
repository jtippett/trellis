...
module Trellis::Providers
  ...
  class Anthropic < Trellis::BaseProvider
    ...
    def prepare_request(operation : Symbol, model : LLMDB::Model, data, opts) : HTTP::Request
      ...
      context = data.as(Trellis::Context)
      url = URI.parse("#{default_base_url}/v1/messages")
      req = HTTP::Request.new("POST", url)
      req.operation = operation
      req.model = model
      req.context = context
      req.options = opts.as(Trellis::Options::Validated)
      req
    end
    ...
    protected def apply_common_headers(req : HTTP::Request) : Nil
      ...
      unless Trellis::Fixture.will_replay?(req, id)
        api_key = Trellis::Keys.resolve(default_env_key, explicit_api_key(req))
    ...
    def encode_body(req : HTTP::Request) : HTTP::Request
      ...
      context = req.context.as(Trellis::Context)
      opts = req.options.as(Trellis::Options::Validated)
    ...
    def encode_object_body(model : LLMDB::Model, context : Trellis::Context,
                           opts : Trellis::Options::Validated,
    ...
    # `stream`, `tools?`.
    def encode_chat_body(model : LLMDB::Model, context : Trellis::Context,
                         opts : Trellis::Options::Validated,
    ...
    private def chat_body_hash(model : LLMDB::Model, context : Trellis::Context,
                               opts : Trellis::Options::Validated,
    ...
    private def partition_system(messages : Array(Trellis::Message)) : {Array(Trellis::Message), Array(Trellis::Message)}
      system = [] of Trellis::Message
      non_system = [] of Trellis::Message
      messages.each do |m|
        if m.role.system?
          system << m
        else
    ...
    private def encode_system(messages : Array(Trellis::Message)) : JSON::Any?
    ...
    private def encode_message(message : Trellis::Message) : Hash(String, JSON::Any)
      case message.role
      when .tool?
    ...
    private def encode_tool_message(message : Trellis::Message) : Hash(String, JSON::Any)
      id = message.tool_call_id
      if id.nil?
        raise Trellis::Error::Invalid::Parameter.new("tool message missing tool_call_id")
      end
    ...
    private def encode_assistant_with_tool_calls(message : Trellis::Message,
                                                 tool_calls : Array(Trellis::ToolCall)) : Hash(String, JSON::Any)
      blocks = [] of JSON::Any
    ...
    # `JSON::Any`.
    private def encode_content(parts : Array(Trellis::ContentPart)) : JSON::Any
    ...
    private def encode_content_part(part : Trellis::ContentPart) : Hash(String, JSON::Any)?
      case part.type
      when .text?
        text = part.text || ""
    ...
    private def encode_tool(tool : Trellis::Tool) : Hash(String, JSON::Any)
      encoded = {
    ...
    private def role_to_wire(role : Trellis::Role) : String
      case role
      when .user?      then "user"
    ...
    def decode_response(req : HTTP::Request, resp : HTTP::Response) : {HTTP::Request, HTTP::Response}
      ...
      finish_reason = Trellis::FinishReason.from_wire(data["stop_reason"]?.try(&.as_s?))
      ...
      parts = [Trellis::ContentPart.text(text)]
      parts << Trellis::ContentPart.thinking(thinking) unless thinking.empty?

      message = Trellis::Message.new(
        Trellis::Role::Assistant,
        parts,
      ...
      input = req.context
      merged_messages = input ? input.messages.dup : [] of Trellis::Message
      merged_messages << message
      merged_tools = input.try(&.tools) || [] of Trellis::Tool
      merged_context = Trellis::Context.new(merged_messages, merged_tools)

      resp.decoded = Trellis::Response.new(
        model: model_name,
    ...
    def decode_stream_event(event : Trellis::SSE::Event) : Array(Trellis::StreamChunk)
      decode_stream_event(event.data)
    end

    # :ditto: convenience overload accepting the raw SSE `data` payload directly.
    def decode_stream_event(data : String) : Array(Trellis::StreamChunk)
      chunks = [] of Trellis::StreamChunk
      stripped = data.strip
      ...
      if parsed["type"]?.try(&.as_s?) == "error"
        ...
        raise Trellis::Error::API::Response.new("Anthropic stream error: #{message}")
      end
      ...
      when "message_start"
        if usage = parsed.dig?("message", "usage")
          if normalized = normalize_stream_usage(usage)
            chunks << Trellis::StreamChunk.meta({"usage" => normalized})
          end
        end
      ...
      when "message_delta"
        if reason = parsed.dig?("delta", "stop_reason").try(&.as_s?)
          chunks << Trellis::StreamChunk.meta(
    ...
    # other block type → `[]`.
    private def decode_block_start(block : JSON::Any?, index : Int32) : Array(Trellis::StreamChunk)
      chunks = [] of Trellis::StreamChunk
      ...
      when "text"
        if text = block["text"]?.try(&.as_s?)
          chunks << Trellis::StreamChunk.text(text) unless text.empty?
        end
      when "thinking"
        if text = block["thinking"]?.try(&.as_s?)
          chunks << Trellis::StreamChunk.thinking(text) unless text.empty?
        end
      when "tool_use"
        id = block["id"]?.try(&.as_s?)
        name = block["name"]?.try(&.as_s?)
        chunks << Trellis::StreamChunk.tool_call_delta(index, id: id, name: name)
      else
    ...
    private def decode_block_delta(delta : JSON::Any?, index : Int32) : Array(Trellis::StreamChunk)
      chunks = [] of Trellis::StreamChunk
      ...
      when "thinking_delta"
        ...
        if text && !text.empty?
          chunks << Trellis::StreamChunk.thinking(text)
        end
      when "input_json_delta"
        if partial = delta["partial_json"]?.try(&.as_s?)
          chunks << Trellis::StreamChunk.tool_call_delta(
    ...
    # `{"", "", []}`.
    private def decode_content(content : JSON::Any?) : {String, String, Array(Trellis::ToolCall)}
      text = String::Builder.new
      thinking = String::Builder.new
      tool_calls = [] of Trellis::ToolCall
      ...
      blocks.try &.each do |block|
        ...
        when "tool_use"
          id = h["id"]?.try(&.as_s?) || Trellis::ToolCall.generate_id
          name = h["name"]?.try(&.as_s?) || ""
          input = h["input"]? || JSON::Any.new({} of String => JSON::Any)
          tool_calls << Trellis::ToolCall.new(id, name, input.to_json)
        else
    ...
    # Decode the Anthropic `usage` object into `Trellis::Usage`. Ports
    ...
    private def decode_usage(usage : JSON::Any?) : Trellis::Usage
      return Trellis::Usage.new unless usage
      h = usage.as_h?
      return Trellis::Usage.new unless h
...
Trellis::Registry.register(Trellis::Providers::Anthropic.new)
