...
module Trellis::Providers
  ...
  class OpenAI < Trellis::BaseProvider
    def id : String
      "openai"
    end
    ...
    def encode_object_body(model : LLMDB::Model, context : Trellis::Context,
      ...
      json_schema = {
        ...
        "schema" => JSON::Any.new(Trellis::Schema.enforce_strict(schema)),
      } of String => JSON::Any
    ...
    def decode_response(req : HTTP::Request, resp : HTTP::Response) : {HTTP::Request, HTTP::Response}
      ...
      finish_reason = Trellis::FinishReason.from_wire(finish_wire)
    ...
    def decode_stream_event(data : String) : Array(Trellis::StreamChunk)
      ...
      if err = parsed["error"]?
        ...
        raise Trellis::Error::API::Response.new("OpenAI stream error: #{message}")
      end
        ...
        choices.each do |choice|
          ...
          if delta
            if content = delta["content"]?.try(&.as_s?)
              chunks << Trellis::StreamChunk.text(content) unless content.empty?
            end

            reasoning = delta["reasoning"]?.try(&.as_s?) ||
                        delta["reasoning_content"]?.try(&.as_s?)
            if reasoning && !reasoning.empty?
              chunks << Trellis::StreamChunk.thinking(reasoning)
            end
    ...
    private def decode_tool_call_delta(tc : JSON::Any) : Trellis::StreamChunk
      ...
      Trellis::StreamChunk.tool_call_delta(
    ...
    # typed blocks.
    private def encode_messages(messages : Array(Trellis::Message)) : Array(JSON::Any)
    ...
    # Decode `message.tool_calls` into `Trellis::ToolCall`s. Each entry is the
    ...
    private def decode_tool_calls(message : JSON::Any?) : Array(Trellis::ToolCall)
      raw = message.try(&.["tool_calls"]?).try(&.as_a?)
      return [] of Trellis::ToolCall unless raw
      raw.map { |entry| Trellis::ToolCall.from_wire(entry) }
    end
...
Trellis::Registry.register(Trellis::Providers::OpenAI.new)
