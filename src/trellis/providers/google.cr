...
module Trellis::Providers
  ...
  class Google < Trellis::BaseProvider
    ...
    def encode_chat_body(model : LLMDB::Model, context : Trellis::Context,
                         opts : Trellis::Options::Validated) : String
    ...
    private def chat_body_hash(model : LLMDB::Model, context : Trellis::Context,
                               opts : Trellis::Options::Validated) : Hash(String, JSON::Any)
      ...
      schema.each do |key, value|
        ...
        when "items"
          if h = value.as_h?
            ...
            raise Trellis::Error::Invalid::Schema.new(
    ...
    private def encode_system_instruction(messages : Array(Trellis::Message)) : JSON::Any?
    ...
    private def encode_text_parts(message : Trellis::Message) : Array(JSON::Any)
      parts = [] of JSON::Any
    ...
    private def encode_assistant_with_tool_calls(message : Trellis::Message,
                                                 tool_calls : Array(Trellis::ToolCall)) : Array(JSON::Any)
      parts = [] of JSON::Any
      message.content.each do |part|
        next unless part.type.text?
        text = part.text || ""
    ...
    private def encode_tool_result_parts(message : Trellis::Message) : Array(JSON::Any)
    ...
    private def encode_generation_config(opts : Trellis::Options::Validated) : JSON::Any?
    ...
    private def message_text(message : Trellis::Message) : String
      String.build do |io|
        message.content.each do |part|
          next unless part.type.text?
          io << (part.text || "")
    ...
    def decode_response(req : HTTP::Request, resp : HTTP::Response) : {HTTP::Request, HTTP::Response}
      ...
      finish_reason = Trellis::FinishReason.from_wire(
        candidate.try(&.["finishReason"]?).try(&.as_s?))
      if finish_reason.stop? && !tool_calls.empty?
        finish_reason = Trellis::FinishReason::ToolCalls
      end
    ...
    private def decode_content(candidate : JSON::Any?) : {String, String, Array(Trellis::ToolCall)}
      ...
      parts.try &.each do |part|
        ...
        if fc = h["functionCall"]?.try(&.as_h?)
          id = fc["id"]?.try(&.as_s?) || Trellis::ToolCall.generate_id
          name = fc["name"]?.try(&.as_s?) || ""
          args = fc["args"]? || JSON::Any.new({} of String => JSON::Any)
          tool_calls << Trellis::ToolCall.new(id, name, args.to_json)
    ...
    # Normalize a Gemini `usageMetadata` object into `Trellis::Usage`. SHARED with
    ...
    private def normalize_google_usage(usage : JSON::Any?) : Trellis::Usage
    ...
    private def extract_chunks_from_parts(parts : Array(JSON::Any)) : Array(Trellis::StreamChunk)
      chunks = [] of Trellis::StreamChunk
      fc_index = 0
      ...
      parts.each do |part|
        ...
        if fc = h["functionCall"]?.try(&.as_h?)
          ...
          chunks << Trellis::StreamChunk.tool_call(name, args,
            {
          ...
          if h["thought"]?.try(&.as_bool?) == true
            ...
            chunks << Trellis::StreamChunk.text(text)
          end
        end
      end
...
Trellis::Registry.register(Trellis::Providers::Google.new)
